    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <SignedOut>
              <SignInButton mode="modal">
                <Button className="relative overflow-hidden bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-lg hover:shadow-blue-500/20 transition-all duration-300 transform hover:-translate-y-0.5">
          ...
          >
            <span
              className={`w-full h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full transition-all ${menuOpen ? "rotate-45 translate-y-2" : ""}`}
            ></span>
            <span
              className={`w-full h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full transition-all ${menuOpen ? "opacity-0" : "opacity-100"}`}
            ></span>
            <span
              className={`w-full h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full transition-all ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`}
            ></span>
        ...
        >
          <nav className="flex flex-col gap-3">
            {navLinks.map((link) => (
              ...
              >
                <div className="px-2 py-2 rounded-lg hover:bg-gray-50">
                  {link.name}
                </div>
              </Link>
            ))}
            ...
            <div className="px-2 py-2 rounded-lg text-gray-700 font-medium">
              Resources
            </div>
            <div className="pl-6 flex flex-col gap-2">
              {["Documentation", "Tutorials", "Blog"].map((item, index) => (
                <Link href="#" key={index} onClick={() => setMenuOpen(false)}>
                  <div className="px-2 py-2 rounded-lg text-gray-600 hover:text-gray-900 hover:bg-gray-50 text-sm transition-colors">
                    {item}
                  </div>
                </Link>
              ))}
